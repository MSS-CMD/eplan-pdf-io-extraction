# -*- coding: utf-8 -*-
"""framework/parsers/iolink.py v3"""
import re
...
def parse(page, pn, config):
    ...
    records = []
    mode = config.get("mode", "connector")
    module_name = ""
    for t, x, y in items:
        if "IOLINK" in t:
            module_name = t.strip()
    ...
    for t, x, y in items:
        if not re.match(r"^[IQ]\d{4}\.\d$", t) or x <= 100:
            continue
        ...
        if mode == "signal" and signal_name:
            desc = signal_name
        else:
        ...
        records.append({
            ...
            "module": module_name, "io_type": io_type, "page": pn, "bit": "",
        })
    return records
