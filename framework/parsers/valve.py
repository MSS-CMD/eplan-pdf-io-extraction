# -*- coding: utf-8 -*-
"""framework/parsers/valve.py - 阀岛解析器 v2 修复奇偶位配对"""
import re
...
def parse(page, pn, config):
    ...
    for addr, ax, ay in q_items:
        ...
        for st, sx, sy in sigs:
            dist = abs(sx - ax) + abs(sy - ay) * 3  # y权重更高
            if dist < best_dist:
                best_dist = dist
                best_sig = st
        records.append({
            "address": addr, "device_tag": "", "description": best_sig,
            "module": config.get("module_name", "阀岛"),
            "io_type": config.get("io_type", "阀岛(FDQ)"), "page": pn, "bit": "",
        })
    return records
